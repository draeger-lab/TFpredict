...
public class GalaxyPredict_iprresult {
	...
	private static ArrayList<String[]> runIPR(String seqfile) {
		...
		System.out.println(iprpath+" -cli -i "+seqfile+" -o "+basedir+"iprres"+" -format raw -goterms -iprlookup -altjobs");
		...
		try {
			...
			proc = rt.exec(iprpath+" -cli -i "+seqfile+" -o "+basedir+"iprres"+" -format raw -goterms -iprlookup -altjobs");
			proc.waitFor();
		} catch (IOException e) {
			e.printStackTrace();
		...
		ArrayList<String[]> IPRentries = readIPRout(basedir+"iprres");
		
		return IPRentries;
	}
	
	
	// read from inputstream and convert ipr results
	...
	// read from file and convert ipr results
	public static ArrayList<String[]> readIPRout(String infile) {
