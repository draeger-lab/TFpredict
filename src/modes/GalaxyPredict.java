...
public class GalaxyPredict {
	...
	private void runInterproScan() {
		...
		// get seq to jobid mapping for gfx theft
		seq2job = runner.getSeq2job();
		...
		try {
			...
			for (int i=0; i<sequence_ids.length; i++) {
				...
				if (predictionPossible.get(seq)) {
				    ...
				    // gfx related
				    if (seq2job.containsKey(seq)) {
						bw.write("<table>\n");
						bw.write("  <tr><th> <img src="+basedir+"/"+seq2job.get(seq)+".visual-png.png/>"+"</th></tr>\n");	
						bw.write("  <tr><th> Illustration generated by InterproScan </th></tr>\n");
						bw.write("</table>\n\n");
						bw.write("<br>\n\n");
				    }
