...
/**
 * <!-- globalinfo-start --> A wrapper class for the liblinear tools (the
 * liblinear classes, typically the jar file, need to be in the classpath to use
 * this classifier).<br/>
...
public class LibLINEARWekaAdapter extends AbstractClassifier implements TechnicalInformationHandler {
	...
	/** SVM solver types */
	public static final Tag[] TAGS_SVMTYPE = { new Tag(SVMTYPE_L2_LR, "L2-regularized logistic regression"),
	...
	public String globalInfo() {
		return "A wrapper class for the liblinear tools (the liblinear classes, typically "
	...
	public Enumeration listOptions() {
		...
		result.addElement(new Option(
				"\tAdd Bias term with the given value if >= 0; if < 0, no bias term added (default: 1)", "B", 1,
				"-B <double>"));
	...
	public void setOptions(String[] options) throws Exception {
		...
		if (tmpStr.length() != 0)
			...
			setSVMType(new SelectedTag(SVMTYPE_L2LOSS_SVM_DUAL, TAGS_SVMTYPE));
	...
	/**
	 * returns whether the liblinear classes are present or not, i.e. whether
	 * the classes are in the classpath or not
	 * 
	 ...
	 * 
	 * @return true if nominal to binary conversion is turned on.
	 */
	...
	/**
	 * Sets the parameters C of class i to weight[i]*C (default 1). Blank
	...
	/**
	 * Gets the parameters C of class i to weight[i]*C (default 1). Blank
	 * separated doubles.
	 * 
	 ...
	 * 
	 * @param o
	 *            the object to set the field for
	 * @param name
	 ...
	 * 
	 * @param o
	 *            the object the field is member of
	 * @param name
	 *            the name of the field
	 * @return the value
	 */
	 ...
	 * 
	 * @param o
	 *            the object to set the array for
	 * @param name
	 *            the name of the field
	 * @param type
	 *            the type of the array
	 * @param length
	 *            the length of the one-dimensional array
	 */
	...
	protected Object getParameters() {
		...
		try {
			...
			result = parameterConstructor.newInstance(solverType, Double.valueOf(m_Cost), Double.valueOf(m_eps));

			if (m_Weight.length > 0) {
				invokeMethod(result, "setWeights", new Class[] { double[].class, int[].class }, new Object[] {
						m_Weight, m_WeightLabel });
			}
		} catch (Exception e) {
			e.printStackTrace();
			result = null;
	...
	public double[] distributionForInstance(Instance instance) throws Exception {
		...
		if (m_ProbabilityEstimates) {
			...
			double v1 = ((Integer) invokeMethod(Class.forName(CLASS_LINEAR).newInstance(), "predictProbability", new Class[] {
					Class.forName(CLASS_MODEL),
					Array.newInstance(Class.forName(CLASS_FEATURENODE), Array.getLength(x)).getClass(),
					Array.newInstance(Double.TYPE, prob_estimates.length).getClass() }, new Object[] { m_Model, x,
			...
			v = ((double[]) invokeMethod(
					...
					"predictDV",
					new Class[] { Class.forName(CLASS_MODEL),
							Array.newInstance(Class.forName(CLASS_FEATURENODE), Array.getLength(x)).getClass() },
					new Object[] { m_Model, x }));
	...
	public void buildClassifier(Instances insts) throws Exception {
		...
		// train model
		m_Model = invokeMethod(Class.forName(CLASS_LINEAR).newInstance(), "train",
				new Class[] { Class.forName(CLASS_PROBLEM), Class.forName(CLASS_PARAMETER) }, new Object[] {
						getProblem(vx, vy, max_index), getParameters() });
	}

	/**
	 * turns on nominal to binary filtering if there are not only numeric
	 * attributes
	 */
	...
	public static void main(String[] args) {
		runClassifier(new LibLINEARWekaAdapter(), args);
	}
}
